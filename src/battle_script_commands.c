...
static void Cmd_jumpifrealmiss(void);
...
static void Cmd_jumpifrealmiss(void)
{
    ...
    if ((gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_MISSED)
     && gBattleStruct->missStringId[gBattlerTarget] == B_MSG_MISSED)
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static bool32 DoSwitchInEffectsForBattler(u32 battler)
{
    ...
    else if (gBattleMons[battler].ability == ABILITY_NEUTRALIZING_GAS && gSpecialStatuses[battler].announceNeutralizingGas == 0)
    {
    ...
    else
    {
        ...
        if ((battlerAbility == ABILITY_TRUANT || IsTruantBattleRule(battler))

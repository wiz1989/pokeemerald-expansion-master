...
static const u32 sNewGameBirch_Gfx[] = INCBIN_U32("graphics/birch_speech/captain_final.4bpp");
static const u32 sUnusedBirchBeauty[] = INCBIN_U32("graphics/birch_speech/unused_beauty.4bpp");
static const u16 sNewGameBirch_Pal[16] = INCBIN_U16("graphics/birch_speech/captain_final.gbapal");
...
{
    ScriptContext_Enable(); //WIP was Context2
    FreezeObjectEvents();
    gPlayerAvatar.preventStep = TRUE;
    SetPlayerAvatarStateMask(PLAYER_AVATAR_FLAG_SURFING);
    PlayerGetDestCoords(&task->tDestX, &task->tDestY);
...
static bool8 RockClimb_WaitStopRockClimb(struct Task *task, struct ObjectEvent *objectEvent)
{
    if (ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
        ...
        gPlayerAvatar.preventStep = FALSE;
        UnfreezeObjectEvents();
        ScriptContext_Stop(); //WIP was Context2
        DestroySprite(&gSprites[objectEvent->fieldEffectSpriteId]);

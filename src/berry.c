...
static u8 CalcBerryYieldInternal(u16 max, u16 min, u8 water)
{
    ...
    // {
        ...
        yield = (extraYield / 2) + min;
        // 25% chance to add an extra yield amount
        if (Random() % 100 < 25)
            yield += min;
        
        // DebugPrintf("CalcBerryYieldInternal: min=%d max=%d rand=%d extraYield=%d yield=%d", min, max, rand, (extraYield / 2) + min, yield);
        return yield;
    // }
}

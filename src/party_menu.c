...
void CB2_ShowPartyMenuForItemUse_Debug(void)
{
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, NULL, &canStopEvo, DO_EVO);
        FreePartyPointers();
        if (!gUseDebugCandy && GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_RareCandy && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
